...
from nlp.dictionary_construct.nmt_en_cmn_dataset import NMTDataset
from nlp.Model.Models import Transformer
from nlp.Model.Translator import Translator
...
def main():
    ...
    # 加载Transformer
    # checkpoint = load_common_kv_from_parllel_model_state_dict(torch.load(path_model, map_location=device, weights_only=False))
    checkpoint = torch.load(path_model, map_location=device, weights_only=False)

    model_opt = checkpoint['settings']
    src_vocab_len, trg_vocab_len = len(train_set.vocab_en), len(train_set.vocab_fra)  # 2927, 3000

    transformer = Transformer(
        src_vocab_len,  # 训练时，若采用了embs_share_weight， 请将src的词表长度设置为trg的词表长度 2024年11月8日
        trg_vocab_len,
    ...
    # 防范多GPU训练情况
    transformer.load_state_dict(load_common_kv_from_parllel_model_state_dict(checkpoint['model']))
...
def load_common_kv_from_parllel_model_state_dict(state_dict_load):
...
if __name__ == "__main__":
    vocab_path_en = './c2e/vocab_en.npy'
    vocab_path_fra = './c2e/vocab_cmn.npy'

    path_txt_train = './c2e/train.txt'
    path_txt_test = './c2e/test.txt'
